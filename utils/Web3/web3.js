...
const web3 = new Web3(
  Web3?.givenProvider?.networkVersion === "1" ? Web3.givenProvider : RPC_URL
);
...
const getContractInstance = memoize(
  (address, abi, account, library, chainId) =>
...
const getEncodedCalls = (callsConfig) => {
  ...
  for (const config of augmentedCallsConfig) {
    ...
    if (typeof config.web3Data === "undefined") {
      throw new Error(
        "multiCall error: config parameter `web3Data` is required"
      );
    }
  }
  ...
  const calls = augmentedCallsConfig.map(
    ({
      ...
      address,
      getContractInstance(address, abi, account, library, chainId)
...
const getDecodedData = ({
  ...
  returnData.map((hexData, i) => {
    ...
    const data =
      outputSignature.length > 1
        ? web3.eth.abi.decodeParameters(
            outputSignature.map(({ type }) => type),
            hexData
          )
...
const multiCall = async (callsConfig) => {
  ...
  const {
    web3Data: { account, library, chainId },
  } = augmentedCallsConfig[0];
  const multicallContract = getContractInstance(
    "0xeefBa1e63905eF1D7ACbA5a8513c70307C1cE441",
    Multicall,
    account,
    library,
