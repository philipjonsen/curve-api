...
const getAaveMarketData = memoize(
  async () =>
    (
      await Request.get(
...
const getAavePoolRewardsInfo = memoize(
  async (gaugesRewardData, CUSTOM_LOGIC_REWARD_CONTRACTS) => {
    const { reserves } = await getAaveMarketData();
    const { [coins.stkaave.coingeckoId]: stkaavePrice } = await getAssetsPrices(
      [coins.stkaave.coingeckoId]
    );

    const aaveGaugesRewardData = gaugesRewardData.filter(({ rewardContract }) =>
      CUSTOM_LOGIC_REWARD_CONTRACTS.includes(rewardContract)
    );
    const poolTokenBalances = await multiCall(
      flattenArray(
        aaveGaugesRewardData.map(({ address }) => {
          const pool = pools.find(
            ({ addresses: { gauge: gaugeAddress } }) =>
              gaugeAddress?.toLowerCase() === address.toLowerCase()
          );
    ...
    return Array.from(Object.entries(tokenBalancesPerPool)).map(
      ([poolId, poolData]) => {
        const pool = pools.getById(poolId);

        const tokenBalances = poolData.map(
          ({ data, metaData: { decimals } }) => data / 10 ** decimals
        );
        const poolTokenApys = poolData.map(
          ({ metaData: { coinAddress } }) =>
            reserves.find(
              ({ aTokenAddress }) =>
                aTokenAddress.toLowerCase() === coinAddress.toLowerCase()
            ).aIncentivesAPY
        );
