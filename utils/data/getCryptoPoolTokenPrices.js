...
const getCryptoPoolTokenPrices = memoize(
  async (account, library, chainId) => {
    const cryptoPools = pools.filter(({ cryptoPool }) => cryptoPool);
    const callsConfig = flattenArray(
      cryptoPools.map(({ id, addresses: { swap, lpToken }, coins }) => [
        {
          address: lpToken,
          abi: ERC20ABI,
    ...
    const referenceAssetPrices = arrayToHashmap(
      await Promise.all(
        cryptoPoolsData
          .filter(({ metaData: { referenceAsset } }) => !!referenceAsset)
          .map(
            ({ metaData: { referenceAsset } }) =>
    ...
    const groupedData = cryptoPoolsData.reduce(
      (
        accu,
        { data, metaData: { poolId, type, referenceAsset, decimals } }
      ) => {
        const { totalSupply, totalBalancesUsdValue } = accu[poolId] || {
          totalSupply: 0,
          totalBalancesUsdValue: 0,
        };

        return {
          ...accu,
          [poolId]: {
            totalSupply: type === "totalSupply" ? data / 1e18 : totalSupply,
            totalBalancesUsdValue:
              type === "balance"
                ? totalBalancesUsdValue +
                  (data / 10 ** decimals) * referenceAssetPrices[referenceAsset]
                : totalBalancesUsdValue,
          },
        };
      },
    ...
    const lpTokenPrices = Array.from(Object.entries(groupedData)).reduce(
      (accu, [poolId, { totalSupply, totalBalancesUsdValue }]) => ({
        ...accu,
