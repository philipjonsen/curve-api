...
export default memoize(
  async ({ blockchainId, gauges }) => {
    ...
    const gaugesData = await multiCall(
      flattenArray(
        sidechainOnlyFactoryGauges.map(({ name, gauge, lpTokenPrice }) => [
          {
            address: gauge,
    ...
    const [gaugesRewardCount, gaugesTotalSupply] = partition(
      gaugesData,
      ({ metaData: { type } }) => type === "rewardCount"
    );

    const rewardTokens = await multiCall(
      flattenArray(
        gaugesRewardCount.map(
          ({ data: rewardCount, metaData: { name, gauge, lpTokenPrice } }) =>
    ...
    const coinAddressesAndPricesMap = await getTokensPrices(
      uniq(
        rewardTokens.map(({ data: rewardTokenAddress }) => rewardTokenAddress)
      ),
    ...
    const rewardsInfo = rewardData.map(
      ({
        ...
        const effectivePeriodFinish = Number(
          typeof periodFinish !== "undefined"
            ? periodFinish
            : periodFinishFallback
        );
        const isRewardStillActive = effectivePeriodFinish > nowTimestamp;
        const totalSupply =
          gaugesTotalSupply.find(({ metaData }) => metaData.name === name)
            .data / 1e18;
        const tokenName = tokenData.find(
          ({ metaData }) => metaData.name === name && metaData.type === "name"
        ).data;
        const tokenSymbol = tokenData.find(
          ({ metaData }) => metaData.name === name && metaData.type === "symbol"
        ).data;
        const tokenDecimals = tokenData.find(
          ({ metaData }) =>
            metaData.name === name && metaData.type === "decimals"
        ).data;
        ...
        return {
          ...
          apy: isRewardStillActive
            ? (((effectiveRate / 1e18) * 86400 * 365 * tokenPrice) /
                totalSupply /
                lpTokenPrice) *
              100
            : 0,
  ...
  },
  {
    ...
    normalizer: ([{ blockchainId, gauges }]) =>
      `${blockchainId}-${gauges.length}`,
  }
);
