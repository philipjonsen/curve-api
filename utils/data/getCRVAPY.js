...
const getCRVAPY = memoize(
  async (userAddress) => {
    const GAUGE_CONTROLLER_ADDRESS =
      "0x2F50D538606Fa9EDD2B11E2446BEb18C9D5846bB";
    const GAUGE_RELATIVE_WEIGHT = "0x6207d866000000000000000000000000";
    const { "curve-dao-token": CRVprice } = await getAssetsPrices([
      "curve-dao-token",
    ]);
    const weightCalls = poolGauges.map((gauge) => [
      GAUGE_CONTROLLER_ADDRESS,
      GAUGE_RELATIVE_WEIGHT + gauge.slice(2),
    ]);
    const aggCallsWeights = await MulticallContract.methods
    ...
    const ratesCalls = flattenArray(
      poolGauges.map((gauge) => [
        [gauge, "0x180692d0"],
        [gauge, "0x17e28089"],
        [gauge, "0x18160ddd"],
      ])
    );
    const aggRates = await MulticallContract.methods
      .aggregate(ratesCalls)
      .call();
    const decodedRate = aggRates[1].map((hex) =>
      web3.eth.abi.decodeParameter("uint256", hex)
    );
    const gaugeRates = decodedRate
      .filter((_, i) => i % 3 === 0)
      .map((v) => v / 1e18);
    const workingSupplies = decodedRate
      .filter((_, i) => i % 3 === 1)
      .map((v) => v / 1e18);
    const virtualPriceCalls = pools
      .filter(({ addresses: { swap } }) => !!swap)
      .map(({ addresses: { swap } }) => [swap, "0xbb7b8b80"]);
    const aggVirtualPrices = await MulticallContract.methods
    ...
    for (const w of decodedWeights) {
      ...
      const {
        id: poolId,
        addresses: { swap: swapAddress },
      } = pool;

      const virtualPrice = decodedVirtualPrices.find(
        (v) => v[0].toLowerCase() === swapAddress.toLowerCase()
      )[1];
      // eslint-disable-next-line no-await-in-loop
      const workingSupply = await getPoolUsdFigure(
        workingSupplies[i],
        pool,
        web3
      );
      ...
      const rate =
        (((gaugeRates[i] * w[1] * 31536000) / workingSupply) * 0.4) /
        virtualPrice;
    ...
    if (userAddress) {
      const wrapper = new GraphQLClient(
        "https://api.thegraph.com/subgraphs/name/sistemico/curve"
      );
      ...
      for (const gaugeBoost of results) {
        const pool = pools.find(
          (pool) =>
            ...
            pool.addresses.gauge.toLowerCase() ===
              gaugeBoost.gauge.id.toLowerCase()
        );
