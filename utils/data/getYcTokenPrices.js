...
const getYcTokenPrices = memoize(
  async (networkSettingsParam) => {
    const ycTokensDataPart1 = await multiCall(
      flattenArray(
        YC_TOKENS.map((address) => [
          {
            address,
            abi: YC_TOKEN_ABI,
    ...
    const underlyingAddressesAndMetadata = ycTokensDataPart1.filter(
      ({ metaData: { type } }) => type === "underlyingAddress"
    );
    const underlyingPrices = await getTokensPrices(
      underlyingAddressesAndMetadata.map(({ data }) => data.toLowerCase())
    );

    const ycTokensDataPart2 = await multiCall(
      underlyingAddressesAndMetadata.map(
        ({ data: underlyingAddress, metaData: { address } }) => ({
    ...
    const ycTokensData = [...ycTokensDataPart1, ...ycTokensDataPart2];

    const groupedData = ycTokensData.reduce(
      (accu, { data, metaData: { type, address } }) => {
    ...
    const ycTokensPrices = arrayToHashmap(
      Array.from(Object.entries(groupedData)).map(([address, data]) => {
        const underlyingBalance =
          data.underlyingBalance / 10 ** data.underlyingDecimals;
