...
export default memoize(
  async ({ factoryGaugesAddresses, blockchainId } = {}) => {
    ...
    if (typeof factoryGaugesAddresses === "undefined") {
      ...
      const factoryGauges = Array.from(Object.values(gauges)).filter(
        ({ factory, side_chain }) => factory && !side_chain
      );
    ...
    const gaugesData = await multiCall(
      flattenArray(
        factoryGaugesAddresses.map((address) => [
          {
            address,
    ...
    const rewardTokens = await multiCall(
      flattenArray(
        gaugesRewardCount.map(({ data: rewardCount, metaData: { address } }) =>
    ...
    const tokenPricesPromise = getTokensPrices(
      uniq(
        rewardTokens.map(
          ({ data: rewardTokenAddress }) =>
            REWARD_TOKENS_REPLACE_MAP[rewardTokenAddress] || rewardTokenAddress
        )
      )
    );
    ...
    const rewardsInfo = rewardData.map(
      ({ data, metaData: { gaugeAddress, rewardTokenAddress } }) => {
        ...
        const totalSupply =
          gaugesTotalSupply.find(
            ({ metaData: { address } }) => address === gaugeAddress
          ).data / 1e18;
        const tokenName = tokenData.find(
          ({ metaData }) =>
            metaData.rewardTokenAddress === rewardTokenAddress &&
            metaData.type === "name"
        ).data;
