...
/* eslint-disable */
const POOL_BALANCE_ABI_UINT256 = [
  {
    gas: 1823,
...
const POOL_PRICE_ORACLE_NO_ARGS_ABI = [
  {
    stateMutability: "view",
    type: "function",
    name: "price_oracle",
...
const POOL_PRICE_ORACLE_WITH_ARGS_ABI = [
  {
    stateMutability: "view",
    type: "function",
    name: "price_oracle",
...
const getEthereumOnlyData = async ({ preventQueryingFactoData }) => {
  ...
  if (!preventQueryingFactoData) {
    const getFactoryV2GaugeRewards = (
      await import("utils/data/getFactoryV2GaugeRewards")
    ).default;
    const getGauges = (await import("pages/api/getGauges")).default;

    [{ gauges: gaugesData }, gaugeRewards] = await Promise.all([
      getGauges.straightCall({ blockchainId: "ethereum" }),
      getFactoryV2GaugeRewards({ blockchainId: "ethereum" }),
    ]);
  }

  const { poolList: mainRegistryPoolList } =
    await getMainRegistryPools.straightCall();
  ...
  const gaugesAssetPrices = await getAssetsPrices(
    Array.from(Object.values(factoryGaugesPoolAddressesAndCoingeckoIdMap))
  );
  const factoryGaugesPoolAddressesAndAssetPricesMap = arrayToHashmap(
    Array.from(Object.entries(factoryGaugesPoolAddressesAndCoingeckoIdMap)).map(
      ([address, coingeckoId]) => [
        address.toLowerCase(),
  ...
  return {
    mainRegistryPoolList: mainRegistryPoolList.map((address) =>
      address.toLowerCase()
    ),
    gaugesDataArray,
    gaugeRewards,
 ...
 */
export default fn(
  async ({ blockchainId, registryId, preventQueryingFactoData }) => {
     ...
     * the pool list only.
     */
    if (typeof preventQueryingFactoData === "undefined") {
      preventQueryingFactoData = false;
    } // Default value
    ...
    if (
      ...
      throw new Error(
        "registryId must be 'factory'|'main'|'crypto'|'factory-crypto'"
      );
    }

    if (
      registryId === "factory" &&
      typeof getFactoryRegistryAddress !== "function"
    ) {
      throw new Error(
        `No getFactoryRegistryAddress() config method found for blockchainId "${blockchainId}"`
      );
    }

    if (
      registryId === "crypto" &&
      typeof getCryptoRegistryAddress !== "function"
    ) {
      throw new Error(
        `No getCryptoRegistryAddress() config method found for blockchainId "${blockchainId}"`
      );
    }

    if (
      registryId === "factory-crypto" &&
      typeof getFactoryCryptoRegistryAddress !== "function"
    ) {
      throw new Error(
        `No getFactoryCryptoRegistryAddress() config method found for blockchainId "${blockchainId}"`
      );
    }
    ...
    const registryAddress =
      registryId === "factory"
        ? await getFactoryRegistryAddress()
        : registryId === "main"
        ? await getRegistry({ blockchainId })
        : registryId === "crypto"
        ? await getCryptoRegistryAddress()
        : registryId === "factory-crypto"
        ? await getFactoryCryptoRegistryAddress()
        : undefined;
    ...
    const POOL_ABI =
      registryId === "factory-crypto" ? factoryCryptoPoolAbi : factoryPoolAbi;

    const REGISTRY_ABI =
      registryId === "factory-crypto"
        ? factoryCryptoRegistryAbi
        : registryId === "crypto"
        ? cryptoRegistryAbi
    ...
    const unfileteredPoolIds = Array(poolCount)
      .fill(0)
      .map((_, i) => i);

    const unfilteredPoolAddresses = await multiCall(
      unfileteredPoolIds.map((id) => ({
        contract: registry,
    ...
    const poolAddresses = unfilteredPoolAddresses.filter(
      (address) => !DISABLED_POOLS_ADDRESSES.includes(address.toLowerCase())
    );
    const poolIds = unfileteredPoolIds.filter(
      (id) =>
        !DISABLED_POOLS_ADDRESSES.includes(
          unfilteredPoolAddresses[id].toLowerCase()
        )
    );

    const ethereumOnlyData =
      blockchainId === "ethereum"
        ? await getEthereumOnlyData({ preventQueryingFactoData })
        : undefined;

    const poolDataWithTries = await multiCall(
      flattenArray(
        poolAddresses.map((address, i) => {
          const poolId = poolIds[i];
          const poolContract = new web3.eth.Contract(
            [...POOL_ABI, ...POOL_TOKEN_METHOD_ABI],
            address
          );
    ...
    const poolData = poolDataWithTries
      .map(({ data, metaData }) => {
        const isLpTokenAddressTry = metaData.type?.startsWith(
          "lpTokenAddress_try_"
        );
        ...
        ? // coinsAddresses doesn’t contain the lp token on factory registry, add it
          poolData.map(({ data, metaData }) => {
            if (metaData.type === "coinsAddresses") {
              const { data: poolImplementationAddress } = poolData.find(
                (d) =>
                  d.metaData.poolId === metaData.poolId &&
                  d.metaData.type === "implementationAddress"
              );

              const implementation = implementationAddressMap.get(
                poolImplementationAddress.toLowerCase()
              );
              ...
              const isUsdMetaPool =
                isMetaPool && implementation.startsWith("metausd");
              const isBtcMetaPool =
                isMetaPool && implementation.startsWith("metabtc");

              const coinsAddresses = [...data];

              const [metaUsdLpTokenAddress, metaBtcLpTokenAddress] = Array.from(
                BASE_POOL_LP_TO_GAUGE_LP_MAP.keys()
              );
    ...
    const lpTokensWithMetadata = tweakedPoolData.filter(
      ({ data, metaData }) =>
        metaData.type === "lpTokenAddress" && data !== ZERO_ADDRESS
    );
    ...
    const lpTokenData =
      lpTokensWithMetadata.length === 0
        ...
        : await multiCall(
            flattenArray(
              lpTokensWithMetadata.map(({ data: address, metaData }) => {
                const lpTokenContract = new web3.eth.Contract(
                  erc20Abi,
                  address
                );
                const poolCoinsCount = tweakedPoolData
                  .find(
                    ({ metaData: { type, poolId } }) =>
                      type === "coinsAddresses" && poolId === metaData.poolId
                  )
                  .data.filter(
                    (coinAddress) =>
                      coinAddress !==
                ...
                const poolAddress =
                  poolAddresses[poolIds.indexOf(metaData.poolId)];
                const poolContractForPriceOracleCall = new web3.eth.Contract(
                  poolHasMultipleOracles
                    ? POOL_PRICE_ORACLE_WITH_ARGS_ABI
                    : POOL_PRICE_ORACLE_NO_ARGS_ABI,
                  poolAddress
                );
    ...
    const augmentedPoolData = [...tweakedPoolData, ...lpTokenData];

    const allCoinAddresses = augmentedPoolData.reduce(
      (accu, { data, metaData: { poolId, type } }) => {
    ...
    const coinAddressesAndPricesMap = await getTokensPrices(
      allCoinAddresses.map(({ address }) => address),
      platformCoingeckoId
    );

    const coinsFallbackPrices = COIN_ADDRESS_COINGECKO_ID_MAP[blockchainId]
      ? await getAssetsPrices(
          Array.from(Object.values(COIN_ADDRESS_COINGECKO_ID_MAP[blockchainId]))
        )
      : {};
    const coinAddressesAndPricesMapFallback = COIN_ADDRESS_COINGECKO_ID_MAP[
      blockchainId
    ]
      ? arrayToHashmap(
          Array.from(
            Object.entries(COIN_ADDRESS_COINGECKO_ID_MAP[blockchainId])
    ...
    const coinData = await multiCall(
      flattenArray(
        allCoinAddresses.map(({ poolId, address }) => {
          // In crypto facto pools, native eth is represented as weth
          const isNativeEth =
            registryId === "factory-crypto"
              ? address.toLowerCase() ===
                allCoins[config.nativeAssetErc20WrapperId].address.toLowerCase()
              : address.toLowerCase() ===
          ...
          const coinContract = isNativeEth
            ? undefined
            : hasByte32Symbol
            ? new web3.eth.Contract(erc20AbiMKR, address)
            : new web3.eth.Contract(erc20Abi, address);

          const poolAddress = poolAddresses[poolIds.indexOf(poolId)];
          const poolContractUint256 = new web3.eth.Contract(
            POOL_BALANCE_ABI_UINT256,
            poolAddress
          );
          const poolContractInt128 = new web3.eth.Contract(
            POOL_BALANCE_ABI_INT128,
            poolAddress
          );
          const coinIndex = poolData
            .find(
              ({ metaData }) =>
                metaData.type === "coinsAddresses" && metaData.poolId === poolId
            )
            .data.findIndex(
              (coinAddress) =>
                coinAddress.toLowerCase() === address.toLowerCase()
            );
              ...
              : [
                  ...
                  },
                  {
                    ...
                    metaData: {
                      ...
                      coinAddress: address,
                      isNativeEth,
                      type: "poolBalanceInt128",
                    },
                    ...networkSettingsParam,
                  },
                ]),
            ...(typeof BASE_POOL_LP_TO_GAUGE_LP_MAP !== "undefined" &&
            BASE_POOL_LP_TO_GAUGE_LP_MAP.has(address)
              ? [
                  {
                    contract: new web3.eth.Contract(
                      erc20Abi,
                      BASE_POOL_LP_TO_GAUGE_LP_MAP.get(address)
                    ),
    ...
    const mergedCoinData = coinData.reduce(
      (
        accu,
        {
          data,
          metaData: { poolId, poolAddress, coinAddress, type, isNativeEth },
        }
      ) => {
        ...
        const coinPrice =
          ...
          (registryId === "factory" &&
            ethereumOnlyData?.factoryGaugesPoolAddressesAndAssetPricesMap?.[
        ...
        accu[key] = {
          ...
          ...(type === "poolBalanceUint256"
            ...
            type === "poolBalanceInt128"
            ? { poolBalance: BN.max(coinInfo.poolBalance, data).toFixed() }
            : type === "poolStakedBalance"
            ? { poolBalance: BN(coinInfo.poolBalance).plus(data).toFixed() }
            : { [type]: data }),
    ...
    const mergedPoolData = augmentedPoolData.reduce(
      (accu, { data, metaData: { poolId, type } }) => {
        ...
        accu[index] = {
          ...
          [type]: type === "priceOracle" ? data / 1e18 : data,
        };
            ...
            flattenArray(
              mergedPoolData.map(
                ({
                  ...
                  if (
                    Number(totalSupply) <
                    SMALL_AMOUNT_UNIT.times(1e18).times(10)
                  ) {
                    return [];
                  } // Ignore empty pools

                  const coinsAddresses =
                    unfilteredCoinsAddresses.filter(isDefinedCoin);
                  const poolContract = new web3.eth.Contract(POOL_ABI, address);

                  return flattenArray(
                    coinsAddresses.map((_, i) => {
                      const iDecimals = Number(decimals[i]);
                      const smallAmount = SMALL_AMOUNT_UNIT.times(
                        BN(10).pow(iDecimals)
                      ).toFixed();
                        ...
                        .map((__, j) => {
                          ...
                          return {
                            ...
                            metaData: {
                              ...
                              i,
                              j,
                              jDivideBy: SMALL_AMOUNT_UNIT.times(
                                BN(10).pow(Number(decimals[j]))
                              ),
                            },
                            ...networkSettingsParam,
                          };
    ...
    const internalPoolsPrices = groupBy(
      rawInternalPoolsPrices.map(
        ({ data, metaData: { poolId, i, j, jDivideBy } }) => {
    ...
    const augmentedData = await sequentialPromiseReduce(
      mergedPoolData,
      async (poolInfo, i, wipMergedPoolData) => {
        const implementation =
          registryId === "factory"
            ? implementationAddressMap.get(
                poolInfo.implementationAddress.toLowerCase()
              )
            : "";

        const isUsdMetaPool =
          implementation.startsWith("metausd") ||
          implementation.startsWith("v1metausd");
        const isBtcMetaPool =
          implementation.startsWith("metabtc") ||
          implementation.startsWith("v1metabtc");
        ...
        const assetTypeName =
          implementation === "plain2eth" ||
          implementation === "plain3eth" ||
          implementation === "plain4eth"
            ? nativeCurrencySymbol.toLowerCase()
            : isBtcMetaPool
            ? "btc"
            : isUsdMetaPool
            ? "usd"
        ...
        const gaugeAddress =
          typeof ethereumOnlyData !== "undefined"
            ? ethereumOnlyData.gaugesDataArray
                .find(
                  ({ swap }) =>
                ...
                ?.gauge?.toLowerCase()
            : undefined;
        const gaugeRewardsInfo = gaugeAddress
          ? ethereumOnlyData.gaugeRewards[gaugeAddress]
          : undefined;
        ...
        return {
          ...
          gaugeRewards:
            typeof gaugeRewardsInfo === "undefined"
              ? undefined
              : gaugeRewardsInfo.map(({ apyData, ...rewardInfo }) => {
                  const gaugeTotalSupply = apyData.totalSupply;
                  const poolTotalSupply = poolInfo.totalSupply / 1e18;
                  const gaugeUsdTotal =
                    (gaugeTotalSupply / poolTotalSupply) * usdTotal;

                  return {
                    ...rewardInfo,
                    apy: apyData.isRewardStillActive
                      ? ((apyData.rate * 86400 * 365 * apyData.tokenPrice) /
                          gaugeUsdTotal) *
                        100
                      : 0,
                  };
            ...
            tvl: sum(
              registryId === "factory"
                ? augmentedData
                    .filter(
                      ({ address }) =>
                        !ethereumOnlyData.mainRegistryPoolList.includes(
                          address.toLowerCase()
                        )
                    )
