...
export default fn(
  async () => {
    const price_feed = await (
      await fetch(
        "https://api.coingecko.com/api/v3/simple/price?ids=tether,ethereum,bitcoin,tether-eurt&vs_currencies=usd"
      )
    ).json();
    ...
    for (const [key, pool] of Object.entries(pools)) {
      const poolC = new web3.eth.Contract(aavePool, pool.address);

      let multiplier =
        pool.type == "stable" || pool.type == "crypto"
          ? 1
          : price_feed[pool.type].usd;
    ...
    const res = await (
      await fetch("https://api.curve.fi/api/getFactoryV2Pools/arbitrum")
    ).json();
    tvl += res.data.tvlAll;
    const factory = {
      tvl: res.data.tvlAll,
