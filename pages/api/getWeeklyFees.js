...
export default fn(
  async () => {
    const feeDistributorAddress = await getFeeDistributor();
    const triPool = "0xbebc44782c7db0a1a60cb6fe97d0b483032ff1c7";

    /* weekly fees from contract */
    const tri_pool = new web3.eth.Contract(tripoolSwapAbi, triPool);
    ...
    for (let i = 0; i < 100; i++) {
      const thisWeekFees = await distributor.methods.tokens_per_week(t).call();
