...
export default fn(
  async () => {
    ...
    for (const [key, pool] of Object.entries(pools)) {
      const poolC = new web3.eth.Contract(poolTwo, pool.address);
      ...
      const virtual_price = await poolC.methods.get_virtual_price().call();
      pools[key].virtual_price = virtual_price;
    }

    const res = await (
      await fetch("https://api.curve.fi/api/getFactoryV2Pools/fantom")
    ).json();
    tvl += res.data.tvlAll;
    const factory = {
      tvl: res.data.tvlAll,
