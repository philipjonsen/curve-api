...
export default fn(
  async ({ blockchainId }) => {
    ...
    const { poolData: mainPoolData } = await getPools.straightCall({
      blockchainId,
      registryId: "main",
    });
    const { poolData: cryptoPoolData } = await getPools.straightCall({
      blockchainId,
      registryId: "crypto",
    });
    const { poolData: factoStablePoolData } = await getPools.straightCall({
      blockchainId,
      registryId: "factory",
    });
    ...
    const sideChainGauges = gauges.filter(
      ({ side_chain: isSideChain, name, is_killed: isKilled }) =>
        isSideChain &&
    ...
    const sideChainGaugesApys = sideChainGauges.map(
      ({
        ...
        const pool = poolData.find(
          ({ address }) => address.toLowerCase() === lcAddress
        );
        if (!pool) {
          throw new Error(
            `Can't find pool data for swap address "${lcAddress}"`
          );
        }
        ...
        const apy = areCrvRewardsStuckInBridge
          ? 0
          : (((rate / 1e18) * (86400 * 365)) / gaugeUsdValue) *
            0.4 *
            crvPrice *
            100;
