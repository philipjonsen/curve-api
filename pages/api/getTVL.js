...
export default fn(
  async () => {
    ...
    const price_feed = await (
      await fetch(
        "https://api.coingecko.com/api/v3/simple/price?ids=stasis-eurs,ethereum,bitcoin,curve-dao-token,chainlink&vs_currencies=usd"
      )
    ).json();
    ...
    const poolList = aggcalls[1].map((hex) =>
    ...
    poolList.map(async (pool_address) => {
      calls.push([
        registryAddress,
        registry.methods.get_balances(pool_address).encodeABI(),
      ]);
      calls.push([
        registryAddress,
        registry.methods.get_coins(pool_address).encodeABI(),
      ]);
      calls.push([
        registryAddress,
        registry.methods.get_lp_token(pool_address).encodeABI(),
      ]);
    });
    ...
    for (var i = 0; i < aggcalls.length; i++) {
      const balances = web3.eth.abi.decodeParameter("uint256[8]", aggcalls[i]);
      i += 1;
      const coins = web3.eth.abi.decodeParameter("address[8]", aggcalls[i]);
      i += 1;
      const lptoken = web3.eth.abi.decodeParameter("address", aggcalls[i]);
    ...
    metaPools.map(async (pool) => {
      const erc20Contract = new web3.eth.Contract(erc20Abi, pool.coin);
    ...
    metaPools.map(async (pool, index) => {
      const decimals = web3.eth.abi.decodeParameter("uint8", aggcalls[index]);
      metaPools[index].decimals = web3.eth.abi.decodeParameter(
        "uint8",
        aggcalls[index]
      );
      metaPools[index].balance =
        metaPools[index].balance / 10 ** metaPools[index].decimals;
    });
    ...
    const allPools = otherPools.concat(metaPools);
    ...
    await Promise.all(
      endPoints.map(async (endPoint) => {
        const res = await (
          await fetch(`https://api.curve.fi/api/${endPoint}`)
        ).json();
        const sideChain = {
