...
export default fn(
  async () => {
    ...
    const mainPoolsGauges = Array.from(Object.values(gauges)).filter(
      ({ side_chain, factory }) => !side_chain && !factory
    );

    const mainPoolsGaugesAddressesAndVersion = mainPoolsGauges.map(
      ({ gauge, name }, i) => ({
        address: gauge,
        version: V0_GAUGES_ADDRESSES.includes(gauge)
          ? null
          : FACTORY_GAUGES_ADDED_TO_MAIN_LIST_ADDRESSES.includes(
              gauge.toLowerCase()
            )
          ? "factory"
          : name === "ankreth"
          ? "v-rewarddata" // Uses the rewardData implementation
          : i < 18
          ? "v1"
          : "v2",
      })
    ...
    const mainPoolsGaugesReferenceAssetCoingeckoIds = uniq(
      mainPoolsGauges
        .map(({ type }) => type)
        .filter((type) => type !== "stable" && type !== "crypto")
    );

    const referenceAssetPricesPromise = getAssetsPrices(
      mainPoolsGaugesReferenceAssetCoingeckoIds
    );
        ...
        mainPoolsGaugesAddressesAndVersion
          ...
          .map(({ version, address }) => [
            ...
            ...(version === "v1"
              ...
              : [
                  ...Array(
                    Number(REWARDS_CONFIG[version].maxRewardCount)
                  ).keys(),
                ].map((rewardIndex) => ({
                  address,
                  abi: GAUGES_PARTIAL_ABI,
    ...
    const gaugesRewardData = mainPoolsGaugesAddressesAndVersion.map(
      ({ address, version }) => {
        if (
          version === null || // No reward contract for those gauges
          version === "factory" // Handled separately with getFactoryV2GaugeRewards() and appended to the result list
        ...
        const rewardContract = gaugesData.find(
          ({ metaData }) =>
            metaData.address === address && metaData.type === "rewardContract"
        ).data;

        const rewardTokens = gaugesData
          .filter(
            ({ metaData }) =>
              metaData.address === address && metaData.type === "rewardToken"
          )
          .map(({ data }) => data)
        ...
        return {
          ...
          version,
          rewardContract:
            rewardContract === ZERO_ADDRESS ? null : rewardContract,
          totalSupply:
            gaugesData.find(
              ({ metaData }) =>
                metaData.address === address && metaData.type === "totalSupply"
            ).data / 1e18,
          rewardTokens:
    ...
    const uniqueRewardTokenAddresses = flattenArray(
      uniq(
        gaugesRewardData
          .map(({ rewardTokens }) => rewardTokens)
          .filter((rewardTokens) => rewardTokens !== null)
      ).map(
        (tokenAddress) =>
          REWARD_TOKENS_REPLACE_MAP[tokenAddress] || tokenAddress
      )
    );
    const tokenPricesPromise = getTokensPrices(uniqueRewardTokenAddresses);

    const filteredGaugeRewardData = gaugesRewardData.filter(
      ({ version, rewardContract }) =>
        version !== null && // No reward contract for those gauges
        version !== "factory" && // Handled separately with getFactoryV2GaugeRewards() and appended to the result list
        rewardContract !== null
    );
    ...
    const rewardAndTokenData = await multiCall(
      flattenArray(
        filteredGaugeRewardData
          .filter(
            ({ rewardContract }) =>
              !CUSTOM_LOGIC_REWARD_CONTRACTS.includes(rewardContract)
          )
          .map(({ address, version, rewardContract, rewardTokens }) => {
            const rewardContractVersion =
              V2_GAUGES_USING_V1_REWARD_CONTRACT.includes(address)
                ? "v1"
                : version;

            return rewardContractVersion === "v-rewarddata"
              ? flattenArray(
                  rewardTokens.map((rewardToken) => [
                    {
                      address: rewardContract,
                      abi: REWARDS_CONFIG[rewardContractVersion]
                        .rewardContractAbi,
    ...
    const [rewardData, tokenData] = partition(
      rewardAndTokenData,
      ({ metaData: { metaType } }) => metaType === "reward"
    );
    const rewardDataPerRewardContractAndToken = groupBy(
      rewardData,
      ({ metaData: { rewardContract, rewardToken } }) =>
        `${rewardContract}-${rewardToken}`
    );

    // Awaiting only here so there's as much work done in parallel as possible
    const [tokenPrices, referenceAssetPrices] = await Promise.all([
      tokenPricesPromise,
      referenceAssetPricesPromise,
    ]);

    const nowTimestamp = getNowTimestamp();
    const rewardsInfo = Array.from(
      Object.values(rewardDataPerRewardContractAndToken)
    ).map((rewardDataForContractToken) => {
      const {
        metaData: { address, rewardToken },
      ...
      if (usesRewardData) {
        const {
          data: { periodFinish, rewardRate },
        } = rewardDataForContractToken[0];

        rate = rewardRate / 1e18;
        rewardPeriodFinish = periodFinish;
      } else {
        const [rewardRate, periodFinish] = [
          rewardDataForContractToken.find(
            ({ metaData: { type } }) => type === "rewardRate"
          ).data,
          rewardDataForContractToken.find(
            ({ metaData: { type } }) => type === "periodFinish"
          ).data,
        ];
      ...
      const { totalSupply } = gaugesRewardData.find(
        (gaugeRewardData) => gaugeRewardData.address === address
      );
      const tokenName = tokenData.find(
        ({ metaData }) =>
          metaData.rewardToken === rewardToken && metaData.type === "name"
      ).data;
      const tokenSymbol = tokenData.find(
        ({ metaData }) =>
          metaData.rewardToken === rewardToken && metaData.type === "symbol"
      ).data;
      const tokenDecimals = tokenData.find(
        ({ metaData }) =>
          metaData.rewardToken === rewardToken && metaData.type === "decimals"
      ).data;
      const lcTokenPriceIndex = (
        REWARD_TOKENS_REPLACE_MAP[rewardToken] || rewardToken
      ).toLowerCase();
      ...
      const referenceAssetPrice =
        gaugeData.type === "stable" ? 1 : referenceAssetPrices[gaugeData.type];
      ...
      return {
        ...
        apy: isRewardStillActive
          ? ((rate * 86400 * 365 * tokenPrice) /
              (totalSupply * referenceAssetPrice)) *
            100
          : 0,
      };
    });

    const [customLogicRewardsInfo, mainPoolsFactoryPoolsRewardsInfo] =
      await Promise.all([
        getAavePoolRewardsInfo(gaugesRewardData, CUSTOM_LOGIC_REWARD_CONTRACTS),
        getFactoryV2GaugeRewards({
          factoryGaugesAddresses: FACTORY_GAUGES_ADDED_TO_MAIN_LIST_ADDRESSES,
        }).then((gaugesRewards) =>
          Array.from(Object.values(gaugesRewards)).map((gaugeRewards) =>
            gaugeRewards.map(({ apyData, ...rewardInfo }) => ({
              ...rewardInfo,
              apy: apyData.isRewardStillActive
                ? ((apyData.rate * 86400 * 365 * apyData.tokenPrice) /
                    (apyData.totalSupply *
                      FACTORY_GAUGES_ADDED_TO_MAIN_LIST_ADDRESSES_REF_ASSET_PRICE[
                        rewardInfo.gaugeAddress
                      ])) *
                  100
                : 0,
            }))
    ...
    const mergedRewardsInfo = [
      ...
      ...flattenArray(
        Array.from(Object.values(mainPoolsFactoryPoolsRewardsInfo))
      ),
    ];

    return {
      mainPoolsGaugeRewards: groupBy(mergedRewardsInfo, "gaugeAddress"),
    };
  },
  {
    maxAge: 5 * 60, // 5 min
