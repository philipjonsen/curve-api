...
export default fn(
  async () => {
    ...
    const price_feed = await (
      await fetch(
        "https://api.coingecko.com/api/v3/simple/price?ids=tether,ethereum,bitcoin,tether-eurt,stasis-eurs,curve-dao-token,convex-finance,tether-gold,spell-token,threshold-network-token&vs_currencies=usd"
      )
    ).json();
    ...
    for (const [key, pool] of Object.entries(cryptoPools)) {
      const poolContract = new web3.eth.Contract(cryptoPoolAbi, pool.address);
      for (let i = 0; i < pool.coins; i++) {
        const balance = await poolContract.methods.balances(i).call();
        cryptoPools[key].tvl +=
          (balance / 10 ** pool.decimals[i]) * price_feed[pool.keys[i]].usd;
      }

      tvl += cryptoPools[key].tvl;

      const tokenContract = new web3.eth.Contract(erc20Abi, pool.token);
