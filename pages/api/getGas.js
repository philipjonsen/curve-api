...
const getBlocknativeData = memoize(
  async () => {
    const {
      blockPrices: [{ baseFeePerGas, estimatedPrices }],
    } = await (
      await fetch(
...
export default fn(
  async () => {
    const [baseFee, fastestGasInfo, fastGasInfo, standardGasInfo, slowGasInfo] =
      await getBlocknativeData();

    const eip1559Gas = {
      base: baseFee * 1e9,
      prio: [fastestGasInfo, fastGasInfo, standardGasInfo, slowGasInfo].map(
        ({ maxPriorityFeePerGas }) => maxPriorityFeePerGas * 1e9
      ),
      max: [fastestGasInfo, fastGasInfo, standardGasInfo, slowGasInfo].map(
        ({ maxFeePerGas }) => maxFeePerGas * 1e9
      ),
    };
