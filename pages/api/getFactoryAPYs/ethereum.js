...
export default fn(
  async ({ version }) => {
    version = version === "crypto" ? "crypto" : Number(version) === 2 ? 2 : 1;
    ...
    const factoryPoolsApiEndpoint =
      version === 1
        ...
        ? "getFactoryV2Pools"
        : version === "crypto"
        ? "getPools/ethereum/factory-crypto"
        : undefined;
    const {
      data: { poolData },
    } = await (
      await fetch(`https://api.curve.fi/api/${factoryPoolsApiEndpoint}`)
    ).json();
    ...
    if (version === 1) {
      ...
      await Promise.all(
        poolData.map(async (pool, index) => {
          ...
          const decimals = [pool.token.decimals, 18, 18, 18];
          let volume = 0;
          ...
          try {
            vPriceDayOldFetch = await poolContract.methods
              .get_virtual_price()
          ...
          try {
            vPriceWeekOldFetch = await poolContract.methods
              .get_virtual_price()
              .call("", latest - DAY_BLOCKS * 7);
          } catch (e) {
      ...
      const {
        ...
        await fetch("https://api.curve.fi/api/getSubgraphData/ethereum")
      ).json();
