...
export default fn(
  async ({ blockchainId }) => {
    ...
    const gaugeRegistry = new web3.eth.Contract(
      GAUGE_REGISTRY_ABI,
      gaugeRegistryAddress
    );
    const gaugeRegistrySidechain = new web3Side.eth.Contract(
      GAUGE_REGISTRY_SIDECHAIN_ABI,
      gaugeRegistryAddress
    );
    ...
    if (
      Number(mirroredGaugeCount) === 0 &&
      Number(unmirroredGaugeCount) === 0
    ) {
      return {
        gauges: [],
      };
    ...
    const unfilteredMirroredGaugeList = await multiCall(
      arrayOfIncrements(mirroredGaugeCount).map((gaugeIndex) => ({
    ...
    const unfilteredUnmirroredGaugeList = await multiCall(
      arrayOfIncrements(unmirroredGaugeCount).map((gaugeIndex) => ({
        address: gaugeRegistryAddress,
    ...
    const gaugesKilledInfo = await multiCall(
      unfilteredGaugeList.map((gaugeAddress) => ({
     ...
     */
    const pendingEmissionsRaw = await multiCall(
      gaugeList.map((gaugeAddress) => ({
        address: gaugeAddress,
    ...
    const pendingEmissions = arrayToHashmap(
      pendingEmissionsRaw.map(({ data, metaData }) => {
        const inflationRate = data / (endOfWeekTs - nowTs);

        return [metaData.gaugeAddress, inflationRate];
      })
    );

    const gaugesDataFromSidechain = await multiCall(
      flattenArray(
        gaugeList.map((gaugeAddress) => {
    ...
    const gaugesDataFromMainnet = await multiCall(
      flattenArray(
        gaugeList.map((gaugeAddress) => {
          const baseConfigData = {
    ...
    const gaugesData = gaugeList.map((gaugeAddress) => {
      const gaugeDataFromSidechain = gaugesDataFromSidechain.filter(
        ({ metaData }) => metaData.gaugeAddress === gaugeAddress
      );
      const gaugeDataFromMainnet = gaugesDataFromMainnet.filter(
        ({ metaData }) => metaData.gaugeAddress === gaugeAddress
      );
      ...
      return {
        address: gaugeAddress,
        ...arrayToHashmap(
          gaugeDataFromSidechain.map(({ data, metaData: { type } }) => [
            type,
            data,
          ])
        ),
        ...arrayToHashmap(
          gaugeDataFromMainnet.map(({ data, metaData: { type } }) => [
            type,
            data,
          ])
        ),
    ...
    const [stablePools, cryptoPools, factoPools, factoCryptoPools] =
      await Promise.all([
        (
          await getPools.straightCall({
            blockchainId,
            registryId: "main",
            preventQueryingFactoData: true,
          })
        ).poolData,
        hasCryptoPools
          ? (
    ...
    const gaugesDataWithPoolAddressAndType = gaugesData
      .map((gaugeData) => {
        const poolInMainRegistry = stablePools.find(
          ({ lpTokenAddress, address }) =>
            lpTokenAddress === gaugeData.lpTokenAddress ||
            address === gaugeData.lpTokenAddress
        );
        const poolInCryptoRegistry = cryptoPools.find(
          ({ lpTokenAddress, address }) =>
            lpTokenAddress === gaugeData.lpTokenAddress ||
            address === gaugeData.lpTokenAddress
        );
        const poolInCryptoFactoRegistry = factoCryptoPools.find(
          ({ lpTokenAddress, address }) =>
            lpTokenAddress === gaugeData.lpTokenAddress ||
            address === gaugeData.lpTokenAddress
        );
        const poolInStableFactoRegistry = factoPools.find(
          ({ lpTokenAddress, address }) =>
            lpTokenAddress === gaugeData.lpTokenAddress ||
            address === gaugeData.lpTokenAddress
        );
        ...
        return {
          ...
          poolAddress: (
            poolInMainRegistry ||
            poolInCryptoRegistry ||
            poolInCryptoFactoRegistry ||
            poolInStableFactoRegistry
          ).address,
          lpTokenPrice:
            (
              poolInMainRegistry ||
            ...
            ((
              ...
              1e18),
          type:
            poolInCryptoFactoRegistry || poolInCryptoRegistry
              ? "crypto"
              : "stable",
        };
      })
    ...
    const poolsVirtualPrices = await multiCall(
      gaugesDataWithPoolAddressAndType.map(({ poolAddress }) => ({
        address: poolAddress,
        abi: factorypool3Abi,
    ...
    const gaugesDataWithPoolVprice = gaugesDataWithPoolAddressAndType.map(
      (gaugeData, index) => ({
        ...gaugeData,
    ...
    const formattedGaugesData = gaugesDataWithPoolVprice.map(
      ({
        ...
        const effectiveInflationRate =
          ...
          (getGaugeWeight > 0 ? pendingEmissions[address] : 0);
    ...
    const sideGaugesRewards = await getFactoryV2SidechainGaugeRewards({
      blockchainId,
