...
export default fn(
  async () => {
    const price_feed = await (
      await fetch(
        "https://api.coingecko.com/api/v3/simple/price?ids=tether,tether-eurt,ethereum,bitcoin&vs_currencies=usd"
      )
    ).json();
    ...
    const res = await (
      await fetch("https://api.curve.fi/api/getFactoryV2Pools/polygon")
    ).json();
    tvl += res.data.tvlAll;
    const factory = {
      tvl: res.data.tvlAll,
