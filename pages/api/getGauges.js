...
export default fn(
  async ({ blockchainId } = {}) => {
    ...
    const gaugeController = new web3.eth.Contract(
      gaugeControllerAbi,
      gaugeControllerAddress
    );

    const exampleGauge = new web3.eth.Contract(
      exampleGaugeAbi,
      "0x7ca5b0a2910B33e9759DC7dDB0413949071D7575"
    );

    const exampleSwap = new web3.eth.Contract(
      swapAbi,
      ...
      if (gauges[key].gauge) {
        calls.push([
          ...
          gaugeController.methods
            .get_gauge_weight(gauges[key].gauge)
            .encodeABI(),
        ]);
        ...
        calls.push([
          gaugeControllerAddress,
          gaugeController.methods
            .gauge_relative_weight(gauges[key].gauge)
            .encodeABI(),
        ]);
        calls.push([
          gauges[key].gauge,
          exampleGauge.methods.inflation_rate().encodeABI(),
        ]);
      }
    });

    const gaugeCalls = [];
    const nonSideChainKeys = [];
    Object.keys(gauges).forEach(function (key) {
      if (!gauges[key].side_chain) {
        gaugeCalls.push([
          gauges[key].gauge,
          exampleGauge.methods.working_supply().encodeABI(),
        ]);
        gaugeCalls.push([
          gauges[key].gauge,
          exampleGauge.methods.inflation_rate().encodeABI(),
    ...
    Object.keys(gauges).forEach(function (key) {
      if (!gauges[key].side_chain) {
        swapCalls.push([
          gauges[key].swap,
          exampleSwap.methods.get_virtual_price().encodeABI(),
        ]);
      }
    });

    let aggswapCalls = await multicall.methods.aggregate(swapCalls).call();
    aggswapCalls = aggswapCalls[1].map((hex) =>
      web3.eth.abi.decodeParameter("uint256", hex)
    );

    const allGaugeWorkDetails = [];
    for (var i = 0; i < aggGaugecalls.length; i++) {
      const working_supply = web3.eth.abi.decodeParameter(
        "uint256",
        aggGaugecalls[i]
      );
      i += 1;
      const inflation_rate = web3.eth.abi.decodeParameter(
        "uint256",
        aggGaugecalls[i]
      );
    ...
    for (var i = 0; i < aggcalls.length; i++) {
      const get_gauge_weight = web3.eth.abi.decodeParameter(
        "uint256",
        aggcalls[i]
      );
      i += 1;
      ...
      // i += 1
      const gauge_relative_weight = web3.eth.abi.decodeParameter(
        "uint256",
        aggcalls[i]
      );
      i += 1;
    ...
    const factoGauges = await sequentialPromiseMap(
      chainsToQuery,
      (blockchainIds) =>
        Promise.all(
    ...
    gauges = {
      ...
      ...arrayToHashmap(
        flattenArray(
          factoGauges.map(({ gauges: blockchainFactoGauges }, i) => {
            const blockchainId = chainsToQuery[i];

            return blockchainFactoGauges
              .filter(({ hasCrv }) => hasCrv)
              .map(
                ({
                  gauge,
                  gauge_data: {
