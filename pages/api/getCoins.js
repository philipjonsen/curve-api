...
export default fn(
  async () => {
    ...
    const registryMain = new web3.eth.Contract(
      registryAbi,
      registryMainAddress
    );
    ...
    /// get factory list
    const factoPools = (
      await (await fetch(`${BASE_API_DOMAIN}/api/getFactoryV2Pools`)).json()
    ).data.poolData;
    let coinList = [];
    for (var i = 0; i < factoPools.length; i++) {
      for (let o = 0; o < factoPools[i].coinsAddresses.length; o++) {
        if (
          factoPools[i].coinsAddresses[o] !==
    ...
    for (let i = 0; i < coinMainCoint; i += 1) {
      calls.push([
        registryMainAddress,
        registryMain.methods.get_coin(i).encodeABI(),
      ]);
    }
    let aggcalls = await multicall.methods.aggregate(calls).call();
    const coinListMain = aggcalls[1].map((hex) =>
    ...
    for (var i = 0; i < fullList.length; i++) {
      ...
      calls.push([
        fullList[i],
        erc20Contract.methods.totalSupply().encodeABI(),
      ]);
    }
    ...
    for (var i = 0; i < aggcalls.length; i++) {
      try {
        const coinSymbol = web3.eth.abi.decodeParameter("string", aggcalls[i]);
        i += 1;
        const coinDecimal = web3.eth.abi.decodeParameter(
          "uint256",
          aggcalls[i]
        );
        i += 1;
        const coinSupply = web3.eth.abi.decodeParameter("uint256", aggcalls[i]);
    ...
    detailedList.sort((a, b) =>
      a.formattedSupply > b.formattedSupply
        ? 1
        : b.formattedSupply > a.formattedSupply
        ? -1
        : 0
    );
